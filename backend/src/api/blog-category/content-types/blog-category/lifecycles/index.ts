...
export default {
  async afterCreate(event: any) {
    ...
    if (slug) {
      revalidationService.addRoutes('blogCategory', [{ slug }]);
  ...
  async afterPublish(event: any) {
    ...
    if (Array.isArray(entries)) {
      for (const entry of entries) {
        if (entry.slug) {
          revalidationService.addRoutes('blogCategory', [{ slug: entry.slug }]);
        }
      }
    } else if (entries?.slug) {
      revalidationService.addRoutes('blogCategory', [{ slug: entries.slug }]);
    }
    revalidationService.addRoutes('sitemap', [{}]);
    await revalidationService.flush();
  },
